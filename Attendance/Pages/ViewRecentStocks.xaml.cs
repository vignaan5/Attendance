...
public partial class ViewRecentStocks : ContentPage
{
    ...
    public ViewRecentStocks(string emp_id, bool yes)
	{
		InitializeComponent();
        dtpicker.IsVisible = true;
        dtpicker.IsEnabled = true;
        this.employee_ID=emp_id;
        get_recent_sold_items(emp_id,"",yes);
	}


	public async void get_recent_sold_items(string employeeID,string DATE,bool yes)
	{
		...
		await Task.Run(async () => {
            ...
            today = dtpicker.Date.ToString("yyyy-M-d");
			recent_sales = dt.get_employee_recent_stock_on_the_day(employeeID,today);
			DateTime yesterday_date = temp.Date;
			yesterday_date = yesterday_date.AddDays(-1);
			var list2 = new List<Dictionary<string, string>>();
			list2 = dt.get_employee_recent_stock_on_the_day(employeeID,dtpicker.Date.AddDays(-1).Date.ToString("yyyy-M-d"));
	...
	{
        get_recent_sold_items(this.employee_ID,"", true);
	}
}